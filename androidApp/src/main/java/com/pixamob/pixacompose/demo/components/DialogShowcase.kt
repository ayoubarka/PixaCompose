...
fun DialogShowcase() {
    ...
    if (showAlert) {
        PixaAlertDialog(onDismissRequest = { showAlert = false }, title = "Information", message = "This is an alert dialog", confirmText = "OK", variant = DialogVariant.Info)
    }

    if (showConfirm) {
        PixaConfirmDialog(onDismissRequest = { showConfirm = false }, title = "Confirm Action", message = "Are you sure you want to proceed?", confirmText = "Confirm", dismissText = "Cancel", onConfirm = { showConfirm = false })
    }

    if (showDestructive) {
        PixaDestructiveDialog(onDismissRequest = { showDestructive = false }, title = "Delete Item", message = "This action cannot be undone", confirmText = "Delete", dismissText = "Cancel", onConfirm = { showDestructive = false })
    }

    if (showCustom) {
        PixaDialog(onDismissRequest = { showCustom = false }, title = "Custom Dialog", message = "This dialog uses custom variant", variant = DialogVariant.Success, confirmText = "Great", dismissText = "Later", onConfirm = { showCustom = false })
    }
}
