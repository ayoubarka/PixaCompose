...
Component files per category: `actions` 11, `display` 13, `feedback` 8, `inputs` 17, `navigation` 5, `overlay` 4, `surfaces` 3. `inputs/Calendar.kt` holds `PixaCalendar`/`PixaHeatmapCalendar` (reusable month-grid/heatmap browsing primitives); `inputs/DatePicker.kt`'s Calendar/HeatMap variants compose them rather than owning grid logic. Card architecture: `surfaces/Card.kt` holds the primitives only — `PixaSurfaceCard` (the Uber Base-aligned container) and the legacy `ConstraintLayout`-based `PixaCard` (kept only because `BottomNavBar`/`Stepper`/`Toast`/`Alert`/`Chart` still depend on it internally). `display/ContentCard.kt` holds the actual card family built on `PixaSurfaceCard` — `PixaContentCard`'s composable anatomy slots plus its archetype presets (`PixaProductCard`, `PixaArticleCard`, `PixaBookingCard`, `PixaTaskCard`, `PixaPinCard`, `PixaAppCard`, `PixaContactCard`, `PixaStatCard`, `PixaActionCard`, `PixaSelectCard`) and `PixaMessageCard` (absorbed into this file as one more preset — no longer a separate `MessageCard.kt`). `display/Card.kt` no longer exists.
...
- **Material 3 UI components still imported in 7 files** — this is the most direct violation of the top-of-file "no Material 3 UI components" constraint, and is now the largest remaining Material surface:
  - `material3.Text` (5 files): `actions/{Tab, IconButton, Chip}.kt`, `navigation/{Drawer, BottomNavBar}.kt`. The replacement is `androidx.compose.foundation.text.BasicText` — `Button.kt` and the whole card family (`surfaces/Card.kt`, `display/ContentCard.kt`) already use it, so there's ample in-repo precedent. `display/Card.kt` is gone (its `PixaCard` primitive moved to `surfaces/Card.kt`, its former `material3.Text` import was dead code from KDoc samples only). `actions/FAB.kt` was migrated off `material3.Text` during the FAB-family rework (removed the inline-label "extended" mode entirely rather than swapping to `BasicText`, since LINE's FAB spec bans inline text on the circular FAB — see `PixaFabPill` for the icon+label, pill-shaped alternative).
...
- **Raw `.dp` literals**: counts drift fast, re-`grep` before trusting a number here. Worst offenders as of the last full pass: `actions/Tab.kt` (42), `feedback/Skeleton.kt` (23), `navigation/TabBar.kt` (12). `display/Card.kt` (formerly 33) no longer exists — its replacement, `display/ContentCard.kt`, tokenizes sizing through `HierarchicalSize` (a handful of `.dp` literals remain for `MessageCardArtwork`'s trailing-width/top-height defaults, each inline-justified as one-off spec values). `navigation/Stepper.kt` and `actions/FAB.kt` are no longer offenders (0 each) — both were listed as worst-offenders previously; `FAB.kt` was cleared during the FAB-family rework (sizes now resolve through `HierarchicalSize.Container`/`Icon`, elevation through `ComponentElevation`).
...
| `cmp-constraintlayout` 0.7.0 | `surfaces/Card.kt` (legacy `PixaCard`), `display/ContentCard.kt` (`PixaMessageCard`'s artwork layout), `navigation/BottomNavBar.kt` | `PixaCard`'s content slot is a `ConstraintLayoutScope` |
